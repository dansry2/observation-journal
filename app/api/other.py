...
def create_movements(data: MovementCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def get_movements(obs_date: date, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    return _build_movement_response(obs_date, db)

@router.put("/movements/{obs_date}", response_model=MovementResponse)
def update_movements(obs_date: date, data: MovementCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def _build_movement_response(obs_date, db):
    ...
    creator = db.query(User).filter(User.id == entries[0].created_by).first() if entries else None
    return {"date": obs_date, "movements": [{"component_name": e.component_name, "from_antenna": e.from_antenna, "to_antenna": e.to_antenna, "note": e.note} for e in entries], "created_by": creator.full_name if creator else None}

@router.post("/antenna-notes", response_model=AntennaNoteResponse)
def create_antenna_notes(data: AntennaNoteCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    for n in data.notes:
...
def get_antenna_notes(obs_date: date, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def _build_antenna_note_response(obs_date, db):
    ...
    creator = db.query(User).filter(User.id == entries[0].created_by).first() if entries else None
    updater = db.query(User).filter(User.id == entries[0].updated_by).first() if entries else None
    return {"date": obs_date, "notes": [{"antenna_code": e.antenna_code, "note": e.note} for e in entries], "created_by": creator.full_name if creator else None, "updated_by": updater.full_name if updater else None}

@router.post("/notes", response_model=DailyNoteResponse)
def create_note(data: DailyNoteCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def get_notes(obs_date: date, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    notes = db.query(DailyNote).filter(DailyNote.date == obs_date).all()
    result = []
    for n in notes:
        creator = db.query(User).filter(User.id == n.created_by).first()
        result.append({"id": n.id, "date": n.date, "title": n.title, "description": n.description, "created_by": creator.full_name if creator else None, "created_at": str(n.created_at)})
    return result
